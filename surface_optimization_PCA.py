...
def compute_mc_feature_attributions_posterior_batched(z_surface, z0, decoder, model, device,
    ...
    del feature_attr_sum, feature_attr_sq_sum, mu, std, z0_flat
    torch.cuda.empty_cache()

    return normalized_attr.cpu(), feature_attr_var.cpu()
...
def compute_attributions_for_all_images():
    ...
    for i, img_tensor in enumerate(tqdm(all_images, desc="Processing images")):
        try:
            ...
            z_surface, z0, v1, v2 = build_surface_fn_from_image_tensor(img_tensor, area=0.7)
            ...
            # Store results
            all_results['normalized_attributions'].append(attr)
            all_results['variances'].append(var)
            all_results['image_indices'].append(valid_indices[i])

            # ✅ COMPREHENSIVE CLEANUP AFTER EACH IMAGE
    ...
    all_results['normalized_attributions'] = torch.stack(all_results['normalized_attributions'])
...
if __name__ == "__main__":
    ...
    print(f"Total images processed: {len(all_attribution_data['normalized_attributions'])}")
    print(f"Attribution tensor shape: {all_attribution_data['normalized_attributions'].shape}")
    print(f"Variance tensor shape: {all_attribution_data['variances'].shape}")
    print(f"Feature dimension per image: {all_attribution_data['normalized_attributions'].shape[1]}")
